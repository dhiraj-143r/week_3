...
const VERDICT_STYLES: Record<string, { bg: string; text: string; label: string; icon: string }> = {
  ...
  SAFE: { bg: "bg-emerald-500/15", text: "text-emerald-400", label: "Safe", icon: "🟢" },
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">Connect Your Inbox</h3>
            <p className="text-sm text-white/40">Auto-scan emails without copy-pasting</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-xs text-white/30 mb-1.5 uppercase tracking-wider">Email Address</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-xs text-white/30 mb-1.5 uppercase tracking-wider">App Password</label>
            <input
              type="password"
              value={password}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <label className="text-xs text-white/30">Scan last</label>
            <select
              value={emailCount}
              onChange={(e) => setEmailCount(Number(e.target.value))}
              className="px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-white text-sm
            ...
            <label className="text-xs text-white/30">emails</label>
          </div>
          ...
          <div className="flex gap-2 ml-auto">
            <button
              ...
              className="px-5 py-2.5 rounded-xl text-sm font-medium
                         ...
                         hover:bg-white/[0.08] transition-all disabled:opacity-30"
            >
        ...
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-white/40">Scanning emails with AI...</span>
              <span className="text-xs text-indigo-400 font-mono">{scanProgress}%</span>
            </div>
      ...
      {summary && (
        ...
        >
          <h4 className="text-sm font-medium text-white/50 uppercase tracking-wider mb-4">Scan Results</h4>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/15 p-4 text-center">
              ...
              <p className="text-xs text-emerald-400/60 mt-1">Safe</p>
            </div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white/80 font-medium truncate">{result.subject}</p>
                      ...
                      <p className="text-xs text-white/20 mt-1 line-clamp-1">{result.snippet}</p>
                    </div>

                    {/* Verdict badge */}
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {result.score !== undefined && (
                        <span className="text-xs text-white/20 font-mono">{result.score}</span>
                      )}
                        ...
                        <div className="px-5 pb-4 pl-14 space-y-3">
                          ...
                          {result.topSignals && result.topSignals.length > 0 && (
                            <div className="space-y-1.5">
                              <p className="text-[10px] text-white/30 uppercase tracking-wider">Top Signals</p>
                              {result.topSignals.map((sig, j) => (
                                <div key={j} className="flex items-start gap-2 text-xs">
                                  ...
                                  <span className="text-white/40">{sig.detail}</span>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          {result.attachmentResults && result.attachmentResults.length > 0 && (
                            <div className="space-y-2">
                              <p className="text-[10px] text-white/30 uppercase tracking-wider">📎 Attachment Scan Results</p>
                              {result.attachmentResults.map((att, k) => {
                                const attStyle = VERDICT_STYLES[att.verdict] || VERDICT_STYLES.SAFE;
                                return (
                                  <div key={k} className={`rounded-lg p-3 border ${attStyle.bg} border-white/[0.06]`}>
                                    ...
                                    <p className="text-[11px] text-white/40">{att.summary}</p>
                                  </div>
                                );
                              })}
                            </div>
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">Inbox Summary</h3>
            <p className="text-sm text-white/40">AI-powered threat overview of your recent emails</p>
          </div>
        </div>

        {/* Day Range Selector */}
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="text-[10px] text-white/30 uppercase tracking-wider mr-1">Last</span>
          {DAY_OPTIONS.map((d) => (
            <button
              key={d}
          ...
          <button
            ...
            className="ml-auto px-5 py-2 rounded-xl text-sm font-medium bg-gradient-to-r from-violet-600 to-purple-600
                       text-white hover:from-violet-500 hover:to-purple-500 shadow-lg shadow-violet-500/20
        ...
        {summaryLoading && (
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-white/30">Scanning last {summaryDays} day{summaryDays > 1 ? "s" : ""}...</span>
          </div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4 text-center">
                <p className="text-2xl font-bold text-white">{inboxSummary.totalEmails}</p>
                <p className="text-[10px] text-white/30 uppercase tracking-wider mt-1">Emails Analyzed</p>
              </div>
            ...
            <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-white/40">Average Risk Score</span>
            ...
            {inboxSummary.timeline.length > 1 && (
              <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4">
                <h4 className="text-xs text-white/30 uppercase tracking-wider mb-3">Daily Breakdown</h4>
            ...
            {inboxSummary.topSenders.length > 0 && (
              <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4">
                <h4 className="text-xs text-white/30 uppercase tracking-wider mb-3">Top Senders</h4>
                <div className="space-y-2">
                  {inboxSummary.topSenders.map((s, i) => (
                    <div key={i} className="flex items-center justify-between py-1">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs text-white/60 truncate">{s.address}</span>
                      </div>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className="text-[10px] text-white/30">{s.count} email{s.count > 1 ? "s" : ""}</span>
                ...
                {inboxSummary.emails.map((em, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`rounded-lg border p-3 ${style.bg} border-white/[0.04]`}
                    >
                      <div className="flex items-start gap-2">
                        <span className="text-sm mt-0.5">{style.icon}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-white/70 font-medium truncate">{em.subject}</p>
